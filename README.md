...
一个不绑定组织、行业或使用场景的 Windows 通用知识库智能体。它把文档检索、模型服务、语音输入输出和可选桌面助手放在一起，管理员可按 `Ctrl+1` 进入配置。

项目现在还是 WPF/.NET 9 桌面应用，重点放在离线可跑、部署简单和对 OpenAI-compatible 服务的兼容上。

仓库只提供源码，不附带 Logo、应用图标、PDF、知识库数据、模型或构建产物。运行后可在管理员界面选择自己的 Logo 与桌宠资源；未配置 Logo 时使用纯文字占位和 Windows 系统应用图标。
